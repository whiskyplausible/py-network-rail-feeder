    ...
    def on_message(self, headers, messages):
        ...
        try:
            ...
            for message in json.loads(messages):
                ...
                if "CA_MSG" in message and message["CA_MSG"]["area_id"] in ["D0"] and message["CA_MSG"]["to"] in [ "1809", "1807", "1802", "1804"]: #2021 south 2018 north
                    show_trains = True
                    ...
                    if message["CA_MSG"]["to"] == "1809":
                        train_text[0] = service_id
                        train_last_seen[0] = time.perf_counter() 
                    elif message["CA_MSG"]["to"] == "1807":
                        train_text[1] = service_id
                        train_last_seen[1] = time.perf_counter() 
                    elif message["CA_MSG"]["to"] == "1802":
                        train_text[2] = service_id
                        train_last_seen[2] = time.perf_counter() 
                    elif message["CA_MSG"]["to"] == "1804":
                        train_text[3] = service_id
                        train_last_seen[3] = time.perf_counter() 
                ...
                if "CA_MSG" in message and message["CA_MSG"]["area_id"] in ["D0"] and message["CA_MSG"]["to"] in [ "1812", "1813", "1592", "1800"]: #train has passed by now
                    this_id = message["CA_MSG"]["descr"]
                    if message["CA_MSG"]["to"] == "1812":
                        train_text[0] = ""
                        train_last_seen[0] = 0
                        print(get_dt(), "train has passed", this_id, message["CA_MSG"]["to"])
                    elif message["CA_MSG"]["to"] == "1813":
                        train_text[1] = ""
                        train_last_seen[1] = 0
                        print(get_dt(), "train has passed", this_id, message["CA_MSG"]["to"])
                    elif message["CA_MSG"]["to"] == "1592":
                        train_text[2] = ""
                        train_last_seen[2] = 0
                        print(get_dt(), "train has passed", this_id, message["CA_MSG"]["to"])
                    elif message["CA_MSG"]["to"] == "1800":
                        train_text[3] = ""
                        train_last_seen[3] = 0
                        print(get_dt(), "train has passed", this_id, message["CA_MSG"]["to"] )

                    if train_text[0] == "" and train_text[1] == "" and train_text[2] == "" and train_text[3] == "":
                        show_trains = False
