        ...
        for message in json.loads(messages):
            if self.view:
                ...
                if set(stanox_list).intersection(["68", "75", "81", "76"]) != set():
                    print("meow", msg["train_service_code"], msg["train_id"][2:6])
                
                #if message['body']['nextstr(message['body']).find("1G21") > -1:
                #    print("hAI!", message['body'])
            else:
    ...
    def on_message(self, headers, messages):
        ...
        for message in data:
            for k, v in message.items():  
                ...
                if self.view and str(new_formed_message).find("2E44") > -1:
